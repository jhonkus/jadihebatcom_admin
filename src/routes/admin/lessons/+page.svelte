<script lang="ts">
	...
	let _quillTextChangeHandler: any = null;
	...
	async function initQuill() {
		...
		try {
			// guard: if there's already an editor reference, do not init again here
			if (editor) {
				console.warn('[lessons] initQuill called but editor already exists — aborting init');
				return;
			}
			...
			// sync back to selectedLesson — store handler so we can remove it on cleanup
			_quillTextChangeHandler = () => {
				try {
			...
			if (typeof editor.on === 'function') {
				editor.on('text-change', _quillTextChangeHandler);
			} else {
				// fallback: attach DOM input listener
				const ql = el.querySelector('.ql-editor');
				if (ql) ql.addEventListener('input', _quillTextChangeHandler);
			}
		} catch (e) {
	...
	async function initializeEditor() {
		if (!browser) return;
		// Ensure previous instance is fully cleaned before creating a new one.
		cleanupEditor();
		await tick();
	...
	function cleanupEditor() {
		try {
			const el = document.getElementById('lesson-content-editor');
			if (editor) {
				// remove the text-change handler if registered
				try {
					if (_quillTextChangeHandler && typeof editor.off === 'function') {
						editor.off('text-change', _quillTextChangeHandler);
					}
				} catch (e) {
					// ignore
				}
				...
				try {
					// attempt to disable editor
					if (typeof editor.disable === 'function') editor.disable();
				} catch (e) {
					/* ignore */
				}

				// clear reference to handler and editor
				_quillTextChangeHandler = null;
				editor = null;
			}

			// Clear DOM generated by Quill (toolbar + editor contents)
			if (el) {
				el.innerHTML = '';
			}
		} catch (e) {
